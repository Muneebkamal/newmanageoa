                    ...
                    <div class="card-header">
                        <div class="row d-flex align-items-center">
                            <div class="col-md-8 d-flex">
                                ...
                                <span>
                                    @if($lead->bundle ==1)
                                        <i class="ri-add-circle-fill" id="bundleShow_{{ $lead->id }}" style="font-size:17px; cursor: pointer;" onclick="toggleBundle({{ $lead->id }}, true)"  title="Show Bundle Items"></i>
                                        <i class="ri-indeterminate-circle-fill" id="bundleHide_{{ $lead->id }}" style="font-size:17px; cursor: pointer; display: none;" onclick="toggleBundle({{ $lead->id }}, false)" title="Hide Bundle Items"></i>
                                        <i class="ri-handbag-fill mx-1" title="Bundle" style="font-size:17px;"></i>
                                    @endif
                                    @if($lead->is_disputed ==1)
                                    <i class="text-danger  ri-indeterminate-circle-fill"  title="Item data may be disbuted" style="font-size:17px;"></i>
                                    @endif
                                    ...
                                    {{ $lead->name }} &mdash;
                                    {{ Carbon\Carbon::parse($lead->date)->format('M jS, Y') }} |
                                    <span><b>{{ $lead->source->list_name }}</b></span>
                                    <br>
                                    <div class="tags">
                                        <div id="leadTag_{{ $lead->id }}" style="display: inline;">
                                        ...
                                        <a class="ms-2 text-primary" style="display: inline;" data-bs-toggle="modal"
                                            ...
                                            onclick="asinNumber('{{ $lead->asin }}', {{ $lead->id }},'{{ $lead->tags }}')"
                            ...
                            <div class="col-md-4">
                                <div class="d-flex justify-content-end">
                                    ...
                                    <button class="btn btn-light me-1"  data-bs-toggle="tooltip" data-bs-placement="top" title="Create Order" onclick="openModal({{ $lead->id }})"><i class="ri-external-link-line"></i></button>
                                    <button class="btn btn-success me-1" onclick="opneBuyListModal({{ $lead->id }})"><i class="ri-money-dollar-box-line"  data-bs-toggle="tooltip" data-bs-placement="top" title="Add to Buy"></i></button>
                                    <button class="btn btn-primary me-1">+Keepa</button>
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#select-tag-modal"
                                            onclick="asinNumber('{{ $lead->asin }}', {{ $lead->id }},'{{ $lead->tags }}')">
                                            <i class="ri-price-tag-3-fill text-primary me-2"></i>Change Tags
                                        </a>
                                        <a class="dropdown-item" data-bs-toggle="modal"
                                            data-bs-target="#exampleModalScrollable"
                                            onclick="leadFind({{ $lead->id }}), fetchSources({{ $lead->source_id }})">
                                            <i class="ri-pencil-line text-primary me-2"></i>Update Lead
                                        </a>
                                        <a class="dropdown-item" onclick="leadDelete({{ $lead->id }})">
                    ...
                    <div class="card-body">
                        <div class="row d-flex justify-content-evenly">
                            <div class="col d-flex justify-content-center">
                                <div class="lead-item text-center">
                                    <span class="">Buy Cost</span><br>
                                    <span class="fw-bold text-dark">${{ $lead->cost ?? 'N/A' }}</span>
                                </div>
                            </div>
                            <div class="col d-flex justify-content-center">
                                <div class="lead-item text-center">
                                    <span class="">FBA Price</span><br>
                                    <span class="fw-bold text-dark">${{ $lead->sell_price ?? 'N/A' }}</span>
                                </div>
                            </div>
                            <div class="col d-flex justify-content-center">
                                <div class="lead-item text-center">
                                    <span class="">Net Profit</span><br>
                                    <span class="fw-bold text-dark">${{ $lead->net_profit ?? 'N/A' }}</span>
                                </div>
                            </div>
                            <div class="col d-flex justify-content-center">
                                <div class="lead-item text-center">
                                    <span class="">ROI</span><br>
                                    <span class="fw-bold text-dark">{{ $lead->roi ?? 'N/A' }}%</span>
                                </div>
                            </div>
                            <div class="col d-flex justify-content-center">
                                <div class="lead-item text-center">
                                    <span class="">90 Day Average</span><br>
                                    <span class="fw-bold text-dark">{{ $lead->bsr ?? 'N/A' }}</span>
                                </div>
                            </div>
                            <div class="col d-flex justify-content-center">
                                <div class="lead-item text-center">
                                    <span class="">Category</span><br>
                                    <span class="fw-bold text-dark">{{ $lead->category ?? 'N/A' }}</span>
                                </div>
                            </div>
                            <div class="col d-flex justify-content-center">
                                <div class="lead-item text-center">
                                    <span class="">Supplier</span><br>
                                    <a href="{{ $lead->url }}">{{ $lead->supplier ?? 'N/A' }}</a>
                                </div>
                            </div>
                            <div class="col d-flex justify-content-center">
                                <a href="{{ $lead->url }}" class="mt-3">
                                    &lt;
                            ...
                            <div class="col d-flex justify-content-center">
                                <div class="lead-item text-center">
                                    <span class="">ASIN</span><br>
                                    <a target="_blank" href="https://www.amazon.com/dp/{{ $lead->asin }}">{{ $lead->asin ?? 'N/A' }}</a>
                                </div>
                            </div>
                        </div>
                        <hr>
                        <div class="row">
                            <div class="col-4 text-center">
                                ...
                                <span class="fw-bold text-dark">{{ $lead->promo ?? 'N/A' }}</span>
                            </div>
                            <div class="col-4 text-center">
                                <span class="">COUPON CODE</span><br>
                                <span class="fw-bold text-dark">{{ $lead->coupon ?? 'N/A' }}</span>
                            </div>
                            <div class="col-4 text-center">
                                <span class="">LEAD NOTES</span><br>
                                <span class="fw-bold text-dark">{{ $lead->notes ?? 'N/A' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                @if($lead->bundle ==1)
                <div class="row" id="bundle_row_{{$lead->id}}" style="display: none;">
                    @forelse ($lead->bundles as $item)
                        <div class="col-md-4">
                            <div class="card child-card mb-5">
                                <div class="card-body">
                                    ...
                                    <p class="card-text text-end"><small class="text-muted">{{ $lead->asin }}</small></p>
                                </div>
                            </div>     
                        </div>    
                    @empty
